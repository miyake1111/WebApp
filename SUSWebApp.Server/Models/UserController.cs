...
using Npgsql;
using SUSWebApp.Models; // プロジェクト名に合わせて変更してください
        ...
        await using (var reader = await cmd.ExecuteReaderAsync())
        {
            while (await reader.ReadAsync())
            {
                users.Add(new User
                {
                    ...
                    retire_date = reader.IsDBNull(reader.GetOrdinal("retire_date")) ? (DateTime?)null : reader.GetDateTime(reader.GetOrdinal("retire_date")),
                    register_date = reader.GetDateTime(reader.GetOrdinal("register_date")),
                    update_date = reader.IsDBNull(reader.GetOrdinal("update_date")) ? (DateTime?)null : reader.GetDateTime(reader.GetOrdinal("update_date")),
